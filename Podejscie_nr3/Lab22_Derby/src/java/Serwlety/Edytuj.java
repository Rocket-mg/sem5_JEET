 ...
 * @author student
 */
@WebServlet(name = "Edytuj", urlPatterns = {"/Edytuj"})
public class Edytuj extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String imie = request.getParameter("imieE");
        String nazwisko = request.getParameter("nazwiskoE");
        String IDpE = request.getParameter("IDpE");
        String kom_ed = "Nie udalo sie edytowac pracownika o ID "+IDpE;
        if(IDpE != null) {
            int wyn = new PracownicyQuery().edytujPracownika(IDpE,imie,nazwisko);
            if(wyn == 1)
            {
                kom_ed = "Edytowano pracownika o ID "+IDpE;
            }
        }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<title>Servlet Edytuj</title>");            
            out.println("</head>");
            out.println("<body>" + kom_ed+"<br />Lista pracowników<br />"); 
